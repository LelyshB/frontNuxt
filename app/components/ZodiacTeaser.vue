<template>
  <!-- Horizontal teaser showcasing all zodiac signs.  A subtle
       horizontal scroll is used to allow the user to browse the
       cards.  A gradient mask on either side fades the edges. -->
  <section class="py-16 px-4" id="zodiac">
              ...
              <div class="relative p-6 glass-surface rounded-2xl tilt-hover shimmer overflow-hidden">
                ...
                <div class="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-opacity duration-300 pointer-events-none">
                  <div class="absolute inset-2 rounded-full border border-amethyst/30">
                    <div
                      class="absolute w-1.5 h-1.5 bg-gradient-to-r from-amethyst to-aurora-teal rounded-full -top-0.5 left-1/2 transform -translate-x-1/2 shadow-glow-violet"
...
// Import the motion composable to detect reduced motion preferences
...
const handleScroll = () => {
  ...
  const cardWidth = 200 // approximate width of each card in pixels
...
onMounted(() => {
  // Center the scroll on the middle item on initial load
